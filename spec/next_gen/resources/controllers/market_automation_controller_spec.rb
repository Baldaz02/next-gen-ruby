...
RSpec.describe NextGen::Controllers::MarketAutomationController do
  let(:data) { { interval: '1h', limit: 50 } }
  let(:controller) { described_class.new(data) }

  describe '#execute_job' do
    it do
      job_instance = instance_double(NextGen::Jobs::MarketAutomationJob)

      expect(NextGen::Jobs::MarketAutomationJob).to receive(:new)
        .with(data)

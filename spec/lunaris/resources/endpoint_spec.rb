...
RSpec.describe Lunaris::Endpoint do
  let(:logger) { instance_double('Lunaris::Config::Logger') }
  ...
  context 'without timestamps' do
    let(:params) do
      {
        controller_name: 'Lunaris::Controllers::MarketAutomationController',
    ...
    it do
      controller_instance = instance_double('Lunaris::Controllers::MarketAutomationController')

      expect(Lunaris::Controllers::MarketAutomationController)
        .to receive(:new).with({})
  ...
  context 'when an error is raised in the controller' do
    ...
    it do
      ...
      allow(Lunaris::Controllers::MarketAutomationController)
        .to receive(:new).and_return(controller_instance)
      allow(controller_instance)
        .to receive(:execute_job)
        .and_raise(error)

      allow(Lunaris::Config::Logger).to receive(:instance).and_return(logger)
